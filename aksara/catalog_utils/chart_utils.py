...
def slice_timeline(df, range_freq, keys_list, value_obj, cur_group, chart_name) :
    timeline = {'DAILY' : 2, 'WEEKLY' : 5}
    group_time = {'WEEKLY' : 'W', 'MONTHLY' : 'M', 'YEARLY' : 'Y'}
    res = {}
    res['TABLE'] = {}
    res['TABLE']['columns'] = { 'x' : 'Date', 'y': chart_name}
    res['TABLE']['data'] = {}
    ...
    for range in range_freq : 
        ...
        if range in group_time : 
            ...
            res[range]['y'] = range_df.groupby(keys_list)['cases_new'].get_group(cur_group).to_list()
